...
{
    struct TASKLEVEL *level = &taskctl->level[task->level];
...
void task_run(struct TASK *task, int level, int priority)
{
    ...
    if (task->flags == 2 AND task->level != level)
    {
        task_remove(task);
    }
    if (task->flags != 2)
...
{
    struct TASKLEVEL *level = &taskctl->level[taskctl->now_lv];
    struct TASK *new_task, *now_task = level->tasks[level->now];
    level->now++;
    if (level->now == level->running)
    {
        level->now = 0;
    ...
    {
        task_switchsub();
        level = &taskctl->level[taskctl->now_lv];
    }
